...
int main(int argc, char **argv) {
    ...
    if (provided < MPI_THREAD_FUNNELED && grid.world == 0)
        std::cerr << "matFact-grid: warning: MPI lacks THREAD_FUNNELED; threads disabled is safest"
                  << std::endl;

    if (argc < 2) {
        if (grid.world == 0) std::cerr << "usage: matFact-grid <instance.in>" << std::endl;
    ...
    if (std::getenv("MATFACT_VERBOSE") != NULL && grid.world == 0)
        std::cerr << "matFact-grid: grid " << grid.Pr << "x" << grid.Pc
