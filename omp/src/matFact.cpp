...
void matFact(std::string inputFileName) {
    ...
    // One parallel region for the whole factorization. Every thread runs the
    // iteration loop; the worksharing loops inside updateLR divide each iteration's
    // work, and the barrier at the end of updateLR's last loop keeps the (sequential)
    // iterations in order.
    #pragma omp parallel default(none) \
        shared(numberOfIterations, numberOfNonZeroElements, numberOfUsers, numberOfItems, numberOfFeatures, \
               nonZeroUserIndexes, nonZeroItemIndexes, A, L, R, StoreL, StoreR, convergenceCoefficient, delta)
    {
    ...
    if (std::getenv("LOG_RESULTS")) {
        std::ofstream logResults("../compare/data/comparison.omp.csv", std::ios::app);
